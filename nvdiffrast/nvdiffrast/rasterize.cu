...
void Rasterize::init(RasterizeGradParams& rast, ProjectGradParams& proj, int width, int height, int depth, bool enableDB) {
    init((RasterizeParams&)rast, proj, width, height, depth, enableDB);
    CUDA_ERROR_CHECK(cudaMalloc(&rast.grad.out, rast.Size()));
    if (enableDB) {
        CUDA_ERROR_CHECK(cudaMalloc(&rast.grad.outDB, rast.Size()));
    }
    rast.grad.proj = proj.grad.out;
}
...
__global__ void RasterizeBackwardKernel(const RasterizeKernelParams rast, const RasterizeKernelGradParams grad) {
    int px = blockIdx.x * blockDim.x + threadIdx.x;
    int py = blockIdx.y * blockDim.y + threadIdx.y;
    int pz = blockIdx.z;
    if (px >= rast.width || py >= rast.height || pz >= rast.depth)return;
    int pidx = px + rast.width * (py + rast.height * pz);

    float4 r = ((float4*)rast.out)[pidx];
    int idx = (int)r.w - 1;
    if (idx < 0) return;
    ...
    unsigned int idx0 = rast.idx[idx * 3];
    unsigned int idx1 = rast.idx[idx * 3 + 1];
    unsigned int idx2 = rast.idx[idx * 3 + 2];
    float2 p0 = ((float2*)rast.proj)[idx0 * 2];
    float2 p1 = ((float2*)rast.proj)[idx1 * 2];
    float2 p2 = ((float2*)rast.proj)[idx2 * 2];
    float w0 = rast.proj[idx0 * 4 + 3];
    float w1 = rast.proj[idx1 * 4 + 3];
    float w2 = rast.proj[idx2 * 4 + 3];
    p0 = w0 * pix - p0;
    p1 = w1 * pix - p1;
    p2 = w2 * pix - p2;
    float2 e0 = p0 - p2;
    ...
    float ia = 1.f / (a + (a > 0 ? 1e-6f : -1e-6f));
