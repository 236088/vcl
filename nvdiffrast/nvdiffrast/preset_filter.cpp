...
void PresetFilter::display() {
	...
	Filter::forward(target_flt1);
	predict1.forward();
	Filter::forward(target_flt2);
	predict2.forward();
	...
	predict1.draw(-.5f, 0.f);
	predict2.draw(0.f, .5f);
	GLbuffer::draw(gl_hr_target, GL_RGB32F, GL_RGB, .5f, -1.f, 1.f, 0.f);
	glFlush();
}

void PresetFilter::update(double dt) {
	loss_sum += Loss::loss(loss);
	loss_sum1 += predict1.getLoss();
	loss_sum2 += predict2.getLoss();
	if ((++step) % CONSOLE_INTERVAL == 0) {
		std::cout << step << "," << loss_sum / CONSOLE_INTERVAL <<  "," << loss_sum1 / CONSOLE_INTERVAL <<  "," << loss_sum2 / CONSOLE_INTERVAL << std::endl;
		file << step << "," << loss_sum / CONSOLE_INTERVAL << "," << loss_sum1 / CONSOLE_INTERVAL << "," << loss_sum2 / CONSOLE_INTERVAL << std::endl;
		loss_sum = 0.f;
		loss_sum1 = 0.f;
		loss_sum2 = 0.f;
	}
