...
void PresetCube::init() {
	...
	Adam::setHyperParams(noaa_adam_pos, 1e-2, 0.9, 0.999, 1e-8);
	Adam::setHyperParams(noaa_adam_color, 1e-2, 0.9, 0.999, 1e-8);
...
void PresetCube::update(double dt) {
	noaa_loss_sum += Loss::loss(noaa_loss);
	predict_loss_sum += Loss::loss(predict_loss);
	if ((++step) % CONSOLE_INTERVAL == 0) {
		std::cout << step << "," << predict_loss_sum / CONSOLE_INTERVAL << "," << noaa_loss_sum / CONSOLE_INTERVAL<<std::endl;
