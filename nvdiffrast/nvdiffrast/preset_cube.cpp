...
void PresetCube::init(int resolution) {
	...
	GLbuffer::init(gl_target, target_aa.kernel.out, resolution, resolution, 3, 15);
	GLbuffer::init(gl_predict, predict_aa.kernel.out, resolution, resolution, 3, 14);
	GLbuffer::init(gl_hr_target, hr_target_aa.kernel.out, 512, 512, 3, 13);
	GLbuffer::init(gl_hr_predict, hr_predict_aa.kernel.out, 512, 512, 3, 12);
}
